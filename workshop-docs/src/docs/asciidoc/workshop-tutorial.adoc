...
image::images/entwicklertag_frankfurt.jpg[scaledwidth="40%", scaledheight="40%",alt="novatec"]
...
```
 git clone git@github.com:andifalk/reactive-spring-security-5-workshop.git
```
...
* **00-reactive-playground**: This project provides a playground to try reactive streams with project reactor
...
To do this we can use the project **00-reactive-playground**.
...
_PasswordEncoder_ that uses an _PasswordEncoder_ with a _state of the art_ encryption algorithm like _BCrypt_ at the time
...
First we need a user having a password that is encrypted using an _outdated_ algorithm. We achieve this by modifying
...
To activate support for automatic password encryption upgrades we need to extend our existing
...
[CAUTION]
====
Never log any sensitive data like passwords, tokens etc., even in encrypted format. Also never put such sensitive data
...
* *initial-library-server*: The reactive version of the library server (almost similar to workshop step _00-library-server_)
...
|===
|Microservice |URL
|Identity Management Service (OKTA)
|https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7[https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7]
...
[[identity-server]]
==== Okta Identity Server

For this workshop the OpenID Connect certified https://www.okta.com/products/customer-identity/authentication/[OKTA identity provider service] is used.
This service provides solutions for authentication, authorization and user administration.
For our purposes we will use this service to issue OpenID Connect compliant JSON web tokens.
...
https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7/.well-known/openid-configuration[Well known OIDC configuration]
...
|https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7
|authorization_endpoint
|Handles authorization, usually asking for credentials and returns an authorization code
|https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7/v1/authorize
|token_endpoint
|Token endpoint (exchanges given authorization code for access token)
|https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7/v1/token
|userinfo_endpoint
|Endpoint for requesting further user information
|https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7/v1/userinfo
|jwks_uri
|Uri for loading public keys to verify signatures of JSON web tokens
|https://dev-667216.oktapreview.com/oauth2/ausjbx4pq5Wg0kZx10h7/v1/keys
|===
...
and the https://docs.spring.io/spring-security/site/docs/5.1.4.RELEASE/reference/html5/#webflux-oauth2-resource-server[Spring Security 5.1 Reference Documentation] on how to implement a resource server.
====
...
* index.html: The main template that is displayed initially
* books.html: This shows the list of books retrieved from the library server
...
Now when you access http://localhost:8081/userinfo[localhost:8081/userinfo] you should be redirected to the OKTA identity server.
