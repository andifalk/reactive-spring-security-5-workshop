...
[TIP]
====
You may look into the spring boot reference documentation https://docs.spring.io/spring-boot/docs/1.5.x/reference/htmlsingle/#boot-features-security-oauth2-resource-server[Spring Boot 1.5 Reference Documentation]
on how to implement an authorization server.
====
...
include::{snippets}/step-05-oauth2-client-gradle-build.adoc[]
...
We will focus on the second possibility and use our own local authorization server.
